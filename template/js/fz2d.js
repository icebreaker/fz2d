...
Fz2D = (function() {
  ...
  Fz2D.query = (function() {
    var hash, k, kk, q, query, v, vv, _i, _len, _ref, _ref1;
    hash = {};
    query = window.location.href.split('?').pop();
    _ref = query.split('&');
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      q = _ref[_i];
      _ref1 = q.split('='), k = _ref1[0], v = _ref1[1];
      kk = unescape(k);
      vv = unescape(v);
      if (/^\d+$/.test(vv)) {
        vv = parseInt(vv);
  ...
  Fz2D.collide = function(o1, o2, callback) {
    var collide, o3, x1, x2, y1, y2, _i, _j, _len, _len1, _ref, _ref1;
...
Fz2D.Loader.Loaders.Audio = (function(_super) {
...
Fz2D.Loader.Loaders.JSON = (function(_super) {
...
Fz2D.Loader.Loaders.Texture = (function(_super) {
...
Fz2D.Loader.Loaders.TextureAtlas = (function(_super) {
...
Fz2D.Plugins.GitHub = (function(_super) {
  ...
  function GitHub(game) {
    ...
    if (_ref = config.position, __indexOf.call(this.positions, _ref) < 0) {
...
Fz2D.Canvas = (function() {
  ...
  function Canvas(w, h, color, selector) {
    this.w = w;
    this.h = h;
...
Fz2D.CanvasWebGL = (function(_super) {
...
Fz2D.TextureMosaic = (function(_super) {
  __extends(TextureMosaic, _super);

  function TextureMosaic(w, h, tw, alpha) {
    var buffer, ctx, i, image_data, twl, x, xx, y, yy, _i, _j, _k, _l;
    ...
    for (i = _j = 1; 1 <= twl ? _j <= twl : _j >= twl; i = 1 <= twl ? ++_j : --_j) {
    ...
    xx = ((w / tw) | 0) - 1;
    yy = ((h / tw) | 0) - 1;
    for (x = _k = 0; 0 <= xx ? _k <= xx : _k >= xx; x = 0 <= xx ? ++_k : --_k) {
      for (y = _l = 0; 0 <= yy ? _l <= yy : _l >= yy; y = 0 <= yy ? ++_l : --_l) {
...
Fz2D.Animation = (function() {
  function Animation(tag, texture, count, delay) {
    var i, w, _i, _ref;
    this.tag = tag;
...
Fz2D.Font = (function() {
  function Font(texture, size, s, e) {
    var c, end, i, start, w, x, xx, y, yy, _i;
    this.texture = texture;
    this.size = size != null ? size : this.texture.h;
    if (s == null) {
      s = ' ';
    }
    if (e == null) {
    ...
    for (c = _i = start; start <= end ? _i <= end : _i >= end; c = start <= end ? ++_i : --_i) {
      i = c - start;
      xx = i % w;
      yy = (i - xx) / w;
...
Fz2D.Object = (function() {
  function Object(x, y, w, h, tag) {
    this.x = x;
    this.y = y;
    this.w = w;
    this.h = h;
    this.tag = tag != null ? tag : 'object';
...
Fz2D.Entity = (function(_super) {
  __extends(Entity, _super);

  function Entity(texture, x, y, w, h, tag) {
    this.texture = texture;
    if (x == null) {
      x = 0;
    }
...
Fz2D.Gui.Button = (function(_super) {
  __extends(Button, _super);

  function Button(x, y, text, font, size, texture_up, texture_down, texture_over) {
    this.font = font;
    this.size = size != null ? size : this.font.size;
...
Fz2D.Gui.Countdown = (function(_super) {
  __extends(Countdown, _super);

  function Countdown(count, x, y, font, size) {
    this.count = count;
    this.font = font;
    this.size = size != null ? size : this.font.size;
...
Fz2D.Gui.Label = (function(_super) {
  __extends(Label, _super);

  function Label(text, x, y, font, size) {
    this.font = font;
    this.size = size != null ? size : this.font.size;
...
Fz2D.Gui.Mouse = (function(_super) {
...
Fz2D.HorizontalScroller = (function(_super) {
...
Fz2D.VerticalScroller = (function(_super) {
